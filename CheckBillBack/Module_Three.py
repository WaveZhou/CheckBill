...
class Module_Three():
    ...
    def add_to_statement_arrive(self, res_date: str, res_account_id: list, institution: str, file_or_rar: str,
                                start_date=None):
        ...
        for res_account in res_account_id:
            ...
            product_type = self.mp.get_one(sql_get_product_type, [res_account])
            ...
            if '/' in res_date:
                res_date = ''.join(res_date.split('/'))
            origin_path_parent = os.path.join(self.settings['origin_path'], institution)
            origin_path = os.path.join(origin_path_parent, file_or_rar)
            target_path_by_product = os.path.join(self.settings['target_path'], '交易日' + res_date, '对账单按产品整理',
                                                  product, belong + account_type)
            target_path_by_institution = os.path.join(self.settings['target_path'], '交易日' + res_date, '对账单按券商整理',
                                                      belong + account_type, product)
            if not file_or_rar.startswith('已处理'):
                res_pro = self.rename_to_new_dir(origin_path, target_path_by_product, file_or_rar, 1, True)
                self.rename_to_new_dir(origin_path, target_path_by_institution, file_or_rar, 1, True)
                ...
                try:
                    if res_pro.num > 0:
                        ...
                        if res_pro.num - 1 == 0:
                            ...
                            self.mp.modify(sql_update_target_filename, [0, res_account, file_or_rar, res_date])
                ...
                if count == len(res_account_id):
                    self.rename_to_new_dir(origin_path, origin_path_parent, '已处理_' + file_or_rar, 2, True)
            ...
            try:
                for institution in os.listdir(self.settings['origin_path']):
                    ...
                    if institution == '托管估值表' or institution == '未知数据' or institution == '清算数据':
                        continue
                    if os.path.isfile(os.path.join(self.settings['origin_path'], institution)):
                        continue
                            ...
                            if file_or_rar.endswith('rar') or file_or_rar.endswith('RAR') or file_or_rar.endswith(
                                ...
                                try:
                                    ...
                                    logger4.output_log({'file_name': bug_out_path,
                                                        'message': '券商账户为{}的文件夹名未在institution_map中匹配到键'.format(
                                                            institution)})
                                ...
                                if res_date is None or res_date == '':
                                    if institution == '中银国际普通账户':
                                        if '股票期权' in file_or_rar:
                                            try:
                                                ...
                                                res_date = datetime.datetime.strptime(end_date_str,
                                                                                      "%Y年%m月%d日").date().strftime(
                                                    '%Y-%m-%d')
                                        ...
                                        if file_or_rar.endswith('txt'):
                                            with open(os.path.join(self.settings['origin_path'], institution,
                                                ...
                                                try:
                                                    res_content = f.read()
                                                    res = re.search(r'对帐日期:.+(\d{4}[-/]?\d{2}[-/]?\d{2})',
                                                                    res_content).group(0)
                                                except AttributeError:
                                        ...
                                        if res_date == '' or res_date is None:
                                            if '股票期权' in file_or_rar:
                                                ...
                                                if file_or_rar.endswith('pdf'):
                                                    res_year = '2022'
                                                    try:
                                                        month_and_day = file_or_rar.split('-')[1].split('.')[0]
                                                    except Exception:
                                                        with pdfplumber.open(
                                                                os.path.join(self.settings['origin_path'], institution,
                                                                             file_or_rar)) as pdf:
                                    ...
                                    # 这个分支里只要把res_date从文件中读取并赋值就好
                                        ...
                                        file_path = os.path.join(self.settings['origin_path'], institution, file_or_rar)
                                        ...
                                        self.rename_to_new_dir(
                                            os.path.join(self.settings['origin_path'], institution, file_or_rar),
                                            os.path.join(self.settings['origin_path'], '未知数据'),
                                            '待处理_' + file_or_rar, 1,
                                            False)
                                        res_date = '0000-00-00'
                                ...
                                if institution in ('未知数据', '手动分类', '清算数据', '申万宏源', '华泰互换', '华创手动'):
                                    ...
                                    if len(res_account_id) != 0:
                                        ...
                                        res_belong_type_json = self.mp.get_one(sql_get_belong_type, [account_id])
                                ...
                                if len(res_account_id) > 1:
                                    ...
                                    if institution == '华创手动':
                                        for res_id in res_account_id:
                                            ...
                                            account_status = self.mp.get_one(sql, [account_id])
                                            if account_status['status'] == '1':
                                                temp_account_id_list.append({'account_id': account_id})
                                        self.add_to_statement_arrive(res_date, temp_account_id_list, institution,
                                                                     file_or_rar)
                                    else:
                                        ...
                                        bug_out_path = bug_out_path + r'\{}-log.txt'.format(date_str)
                                        logger3.output_log(
                                            {'file_name': bug_out_path,
                                             'message': '文件名{}在对账单应到表中匹配了多个券商账户'.format(file_or_rar)})

                                elif len(res_account_id) == 1:
                                    # 在应到表中匹配到了券商账户id,在实到表中做新增

                                    self.add_to_statement_arrive(res_date, res_account_id, institution, file_or_rar,
                                                                 begin_date)
                                    ...
                                    self.rename_to_new_dir(old_path, target_path, '未匹配_' + file_or_rar, 2, False)
                ...
                for institution in os.listdir(self.settings['origin_path']):
                    ...
                    if os.path.isfile(os.path.join(self.settings['origin_path'], institution)):
                        continue
                    for file in os.listdir(os.path.join(self.settings['origin_path'], institution)):
